...
@SpringComponent
@UIScope
public class EditeurEditeur extends VerticalLayout implements KeyNotifier {
	...
	public EditeurEditeur(EditeurService service) {
		...
		cancel.addClickListener(e -> editEditeur(auteur));
		setVisible(false);
	}

	void delete() {
		auteurService.deleteEditeurById(auteur.getId());
		changeHandler.onChange();
	}

	void save() {
		auteurService.saveEditeur(auteur);
		changeHandler.onChange();
	}
	...
	public final void editEditeur(Editeur a) {
		...
		if (persisted) {
			...
			auteur = auteurService.getEditeurById(a.getId());
		}
		else {
			auteur = a;
		}
