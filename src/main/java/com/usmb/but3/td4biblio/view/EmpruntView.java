...
public class EmpruntView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public EmpruntView(EmpruntService empruntService, EmpruntEditor editor) {
        ...
        grid.addColumn(
                e -> e.getUtilisateur() != null ? e.getUtilisateur().getNom() + " " + e.getUtilisateur().getPrenom()
                        : "")
        ...
        grid.addColumn(new ComponentRenderer<>(e -> {
            ...
            // Si le livre n'est pas rendu ET que la date de fin est strictement avant
            // aujourd'hui
            if (e.getDateRendu() == null && e.getDateFinPrevue() != null
                    && e.getDateFinPrevue().isBefore(LocalDate.now())) {
